...
describe("GitHubSourceResolver", () => {
  it("resolves a release to an exact commit", async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ headers: expect.objectContaining({ "User-Agent": "saleor-yard" }) }),
    );
  });
  ...
  it("rejects private pull request sources", async () => {
    ...
    ).rejects.toMatchObject<Partial<YardError>>({ code: "unsupported_private_source" });
  });
  ...
  it("returns a plain not-found error", async () => {
    ...
    ).rejects.toMatchObject<Partial<YardError>>({ code: "source_not_found" });
  });
});
